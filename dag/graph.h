...
class Node;
struct Bargs {
    Bargs(Node* n, std::shared_ptr<dag::GraphContext> ctx) : node(n), context(ctx) {
    }
    Node* node;
    std::shared_ptr<dag::GraphContext> context;
};
class Node {
    public:
    ...
    void run(std::shared_ptr<dag::GraphContext> context);
    virtual int do_service(std::shared_ptr<dag::GraphContext> context) noexcept;
    virtual bool skip(std::shared_ptr<dag::GraphContext> context);
    bool notify(std::shared_ptr<dag::GraphContext> context);
    ...
    void set_name(const std::string& name) {
        this->name = name;
        recorder = std::make_shared<bvar::LatencyRecorder>
            ("predictor_" + name + "latency");
    }
    ...
    void run_output_nodes_if_ready(std::shared_ptr<dag::GraphContext> context);
...
class Graph {
  ...
  int run(brpc::Controller* cntl, const Request* request,
        Response* response,
        google::protobuf::Closure* done) {
      // 1. 构造图的上下文
      auto context = std::make_shared<dag::GraphContext>(cntl, request, response, done);
      context->Init();
      ...
      for (auto node : all_nodes) {
          ...
          if (it == context->node_input_num_map.end()) {
              ...
              // VLOG_APP(ERROR) << "node: " << node->get_name() << " input_num:" << context->node_input_num_map[node]->load();
          }
          // node_timecost_monitor->insert({node->get_name(), std::make_shared<dag::NodeTimeCostMonitor>()});
      }
      // 2. run
