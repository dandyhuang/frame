...
class Node;
struct Bargs {
    Bargs(Node* n, std::shared_ptr<dag::Context> ctx) : node(n), context(ctx) {
    }
    Node* node;
    std::shared_ptr<dag::Context> context;
};
class Node {
    public:
    ...
    void run(std::shared_ptr<dag::Context> context);
    virtual int do_service(std::shared_ptr<dag::Context> context) noexcept;
    virtual bool skip(std::shared_ptr<dag::Context> context);
    bool notify(std::shared_ptr<dag::Context> context);
    ...
    void run_output_nodes_if_ready(std::shared_ptr<dag::Context> context);
...
class Graph {
  ...
  int run(brpc::Controller* cntl, const Request* request,
      ...
      // 1. 构造图的上下文
      auto context = std::make_shared<dag::Context>(cntl, request, response, done);
      context->init();
