...
void NodeManager::add_nodes_conf(const std::string& file_path) {
    ...
    do {
        ...
        if (!transport.empty()) {
            if (!GtransportManager::instance().get_transport(transport) && !client::RedisClientManager::instance()->get_client(transport)) {
                VLOG_APP(ERROR) << "conf error !! transport_name:" << transport << " not exist in gtransport and redis client";
                std::cout << "conf error !! transport_name:" << transport << " not exist in gtransport and redis client" << std::endl;
            }
        }
        auto next_node_conf_ptr = std::make_shared<dag::ConfigXml>();
