...
#include "hash_map.h"
#include "worker_common/thread_pool.h"
...
class Timer {
 ...
 private:
    ...
    while (tm_running.load()) {
      ...
      if (chrono::duration_cast<chrono::milliseconds>(diff).count() > 0) {
        ...
        if (s.tn_is_period) {
          ...
          if (s.tn_repeated_num > 0) {
            s.tn_tm_point = chrono::high_resolution_clock::now() + s.tn_period;
            s.tn_repeated_num--;
            tm_queue.push(s);
          } else if (s.tn_repeated_num == -1) {
            s.tn_tm_point = chrono::high_resolution_clock::now() + s.tn_period;
            tm_queue.push(s);
          }
        }
        lock.unlock();
        tm_thread_pool.enqueue(std::move(s.tn_callback));
        // tm_thread_pool.enqueue(std::move(s.tn_callback), args)
      }
    }
  }
  ...
  worker::common::ThreadPool tm_thread_pool;
  atomic<int> tm_id;
