...
class Semaphore : Noncopyable {
 public:
  Semaphore(uint32_t count = 0) { sem_init(&m_sem, 0, count); }
...
class WriteScopedLock {
 public:
  WriteScopedLock(T& m) : m_mutex(m) { lock(); }
...
class ReadScopedLock {
 public:
  ReadScopedLock(T& m) : m_mutex(m) { lock(); }
...
class Mutex : Noncopyable {
 public:
  ...
  int trylock() { pthread_mutex_trylock(&m_mutex); }
...
class SpinLock : Noncopyable {
 public:
  ...
  int trylock() { pthread_spin_trylock(&m_mutex); }
...
// do write something 
{
  RWMutex::WriteLock wr_locker(rw_mutex);
}

// do read something
{
  RWMutex::ReadLock rd_locker(rw_mutex);
}
*/
