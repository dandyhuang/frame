...
public:
    ...
    // zeus add
    bool WriteByteArrayRedisCommand(RedisResult &result, 
        char* key, size_t key_len, char* value, size_t vlaue_len, int expire);
    // zeus add 
    bool ReadByteArrayRedisCommand(RedisResult &result, char* key, size_t key_len);
...
private:
    ...
    // zeus add
    uint32_t GetKeySlotIndex(const char* key, size_t len);
    RedisConnection *FindNodeConnection(const char *key);
    // zeus add
    RedisConnection *FindNodeConnection(const char *key, size_t len);
