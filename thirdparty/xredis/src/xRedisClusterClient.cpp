...
// zeus add
uint32_t xRedisClusterClient::GetKeySlotIndex(const char* key, size_t len)
{
    if (NULL != key) {
...
// zeus add
RedisConnection *xRedisClusterClient::FindNodeConnection(const char *key, size_t len)
{
    if (!mClusterEnabled) {
        return GetConnection(0);
    }
    // printf("key:%s \r\n", key);
    uint32_t slot_id = GetKeySlotIndex(key, len);
    uint32_t index = FindNodeIndex(slot_id);
    return GetConnection(index);
}

// zeus add
bool xRedisClusterClient::ReadByteArrayRedisCommand(RedisResult &result, char* key, size_t key_len)
{
    bool bRet = false;
    RedisConnection *pRedisConn = NULL;
    if(0==key_len) {
    ...
    pRedisConn = FindNodeConnection(key, key_len);

    redisReply *reply = redisCommand(pRedisConn->mCtx, "GET %b", key, key_len);
...
// zeus add 
bool xRedisClusterClient::WriteByteArrayRedisCommand(RedisResult &result,  
    char* key, size_t key_len, char* value, size_t value_len, int expire)
{
    bool bRet = false;
    RedisConnection *pRedisConn = NULL;
    if(0==key_len) {
    ...
    redisReply *reply = redisCommand(pRedisConn->mCtx, "SETEX %b %d %b", key, key_len, expire, value, value_len);
