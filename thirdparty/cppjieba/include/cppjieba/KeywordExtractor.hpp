...
class KeywordExtractor {
 public:
  ...
  void init(const DictTrie* dictTrie, 
        const HMMModel* model,
        const string& idfPath, 
        const string& stopWordPath) {
    segment_.init(dictTrie, model);
    LoadIdfDict(idfPath);
    LoadStopWordDict(stopWordPath);
  }
  ...
  void FilterByStopWords(vector<cppjieba::Word>& words_in) const {
      int in_size = words_in.size();
      int start = 0;
      int end = in_size - 1;
      while (start <= end) {
          if (stopWords_.find(words_in[start].word) != stopWords_.end()) {
              std::swap(words_in[start], words_in[end]);
              end--;
              continue;
          }
          start++;
      }
      words_in.resize(start);
  }
  void FilterByStopWords(vector<string>& words_in) const {
      int in_size = words_in.size();
      int start = 0;
      int end = in_size - 1;
      while (start <= end) {
          if (stopWords_.find(words_in[start]) != stopWords_.end()) {
              std::swap(words_in[start], words_in[end]);
              end--;
              continue;
          }
