...
#include "thirdparty/monitor_sdk_common/system/net/domain_resolver.h"
...
{
    common::HostEntry host_entry1;
    common::HostEntry host_entry2;
    common::HostEntry host_entry3;
    int error;
    if (common::DomainResolver::Query("www.qq.com", &host_entry1, &error))
    {
    ...
    {
        std::cout << "Query error: " << common::DomainResolver::ErrorString(error) << '\n';
    }
...
TEST(DomainResolver, ResolveIpAddress)
{
    ...
    int error;
    if (common::DomainResolver::ResolveIpAddress("www.qq.com", &addresses, &error))
    {
        std::cout << "Query success:\nIP:";
...
TEST(DomainResolver, Invalid)
{
    std::vector<IpAddress> addresses;
    int error;
    EXPECT_FALSE(common::DomainResolver::ResolveIpAddress("non-exist.domaon...", &addresses, &error));
    EXPECT_NE(0, error);
    EXPECT_TRUE(common::DomainResolver::IsFatalError(error));
    EXPECT_EQ("Unknown host", common::DomainResolver::ErrorString(error));
}
