...
#include "thirdparty/monitor_sdk_common/system/process/sub_process.h"
...
#include "thirdparty/monitor_sdk_common/encoding/shell.h"
    ...
    if (pid == 0) {
        ...
        if (nread == sizeof(error)) {
            LOG(ERROR) << "Start process " << args[0] << " error: " << error
                       << ", " << strerror(error);
            COMMON_EINTR_IGNORED(waitpid(pid, NULL, 0));
            return false;
        }
        m_pid = pid;
        m_exit_code = 0;
