...
#ifndef COMMON_SYSTEM_THREADING_MUTEX_H
#define COMMON_SYSTEM_THREADING_MUTEX_H
...
class MutexBase
{
    COMMON_DECLARE_UNCOPYABLE(MutexBase);
protected:
...
public:
    void Lock()
    {
        COMMON_CHECK_PTHREAD_ERROR(pthread_mutex_lock(&m_mutex));
        AssertLocked();
    }

    bool TryLock()
    {
        return COMMON_CHECK_PTHREAD_TRYLOCK_ERROR(
    ...
    {
        AssertLocked();
        COMMON_CHECK_PTHREAD_ERROR(pthread_mutex_unlock(&m_mutex));
        // NOTE: can't check unlocked here, maybe already locked by other thread
    }
private:
    friend class ::common::ConditionVariable;
...
#endif // COMMON_SYSTEM_THREADING_MUTEX_H
