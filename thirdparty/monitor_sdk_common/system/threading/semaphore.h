...
#ifndef COMMON_SYSTEM_THREADING_SEMAPHORE_H
#define COMMON_SYSTEM_THREADING_SEMAPHORE_H
...
public:
    ...
    {
        COMMON_CHECK_POSIX_ERROR(COMMON_EINTR_IGNORED(sem_wait(sem)));
    }
    ...
    {
        COMMON_CHECK_POSIX_ERROR(sem_post(sem));
    }

    static unsigned int GetValue(const sem_t* sem)
    {
        int value;
        COMMON_CHECK_POSIX_ERROR(sem_getvalue(const_cast<sem_t*>(sem), &value));
        return value;
    }
};
...
class Semaphore
{
    COMMON_DECLARE_UNCOPYABLE(Semaphore);

public:
    explicit Semaphore(unsigned int value)
    {
        COMMON_CHECK_POSIX_ERROR(sem_init(&m_sem, false, value));
    }

    ~Semaphore()
    {
        COMMON_CHECK_POSIX_ERROR(sem_destroy(&m_sem));
    }
...
{
    COMMON_DECLARE_UNCOPYABLE(NamedSemaphore);
...
#endif // COMMON_SYSTEM_THREADING_SEMAPHORE_H
