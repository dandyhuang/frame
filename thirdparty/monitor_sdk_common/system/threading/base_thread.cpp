...
#include "thirdparty/monitor_sdk_common/system/threading/base_thread.h"
...
{
    COMMON_CHECK_PTHREAD_ERROR(DoStart());
}
...
bool BaseThread::TryStart()
{
    ...
    if (error == 0)
        return true;
    if (error != EAGAIN)
        COMMON_CHECK_PTHREAD_ERROR(error);
    return false;
}

bool BaseThread::DoDetach()
{
    COMMON_CHECK_PTHREAD_ERROR(pthread_detach(m_handle));
    m_handle = HandleType();
    m_id = -1;
    return true;
}
...
{
    assert(IsJoinable());
    COMMON_CHECK_PTHREAD_ERROR(pthread_join(m_handle, NULL));
    m_handle = pthread_t();
    m_id = -1;
    return true;
}
