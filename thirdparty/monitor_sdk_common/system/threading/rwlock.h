...
#ifndef COMMON_SYSTEM_THREADING_RWLOCK_H
#define COMMON_SYSTEM_THREADING_RWLOCK_H
...
class RwLock
{
    COMMON_DECLARE_UNCOPYABLE(RwLock);
public:
...
public:
    ...
    {
        CheckValid();
        COMMON_CHECK_PTHREAD_ERROR(pthread_rwlock_rdlock(&m_lock));
    }

    bool TryReaderLock()
    {
        CheckValid();
        return COMMON_CHECK_PTHREAD_TRYLOCK_ERROR(pthread_rwlock_tryrdlock(&m_lock));
    }

    void WriterLock()
    {
        CheckValid();
        COMMON_CHECK_PTHREAD_ERROR(pthread_rwlock_wrlock(&m_lock));
    }

    bool TryWriterLock()
    {
        CheckValid();
        return COMMON_CHECK_PTHREAD_TRYLOCK_ERROR(pthread_rwlock_trywrlock(&m_lock));
    }

    void Unlock()
    {
        CheckValid();
        COMMON_CHECK_PTHREAD_ERROR(pthread_rwlock_unlock(&m_lock));
    }
...
private:
    void CheckValid() const
    {
        ...
        if (memcmp(&m_lock.__data.__flags,  "\xFF\xFF\xFF\xFF", 4) == 0)
            COMMON_CHECK_ERRNO_ERROR(EINVAL);
#endif
    }
...
#endif // COMMON_SYSTEM_THREADING_RWLOCK_H
