...
template <typename T>
struct type_tester_fp
{
    static void run(const char* type)
    {
        Atomic<T> n = 1.0f;
        ASSERT_DOUBLE_EQ(1, n);

        T t;

        t = (n = 2);
        ASSERT_DOUBLE_EQ(2, n);
        ASSERT_DOUBLE_EQ(2, t);
        ...
        n = 0;
        t = n.Exchange(4);
        ASSERT_DOUBLE_EQ(0, t);
        ASSERT_DOUBLE_EQ(4, n);

        n = 4;
        ASSERT_TRUE(n.CompareExchange(4, 5, &t));
        ASSERT_DOUBLE_EQ(4, t);
        ASSERT_DOUBLE_EQ(5, n);

        n = 5;
        ASSERT_FALSE(n.CompareExchange(4, 6, &t));
        ASSERT_DOUBLE_EQ(5, t);
        ASSERT_DOUBLE_EQ(5, n);
    }
};

template <>
struct type_tester<float> : public type_tester_fp<float>
{
};

template <>
struct type_tester<double> : public type_tester_fp<double>
{
};
