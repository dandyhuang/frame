...
ScheduledExecutor::ScheduledExecutor(int pool_size) {
...
void ScheduledExecutor::Schedule(const std::function<void (bool *)>& callback, int initial_delay) {
    TouchThreadPool();

    if (terminated_) return;

    auto task_func = [this, callback, initial_delay] {
...
void ScheduledExecutor::ScheduleWithFixedDelay(const std::function<void (bool *)>& callback,
...
void ScheduledExecutor::ScheduleWithDynamicDelay(const std::function<void (bool *)>& callback,
...
void ScheduledExecutor::Restart() {
    Shutdown();
    thread_pool_ = nullptr;
    TouchThreadPool();
}

bool ScheduledExecutor::IsTerminated(){
    return terminated_;
}

void ScheduledExecutor::TouchThreadPool() {
