...
int main(int argc, char* argv[]){
    ...
    props[PropertyKeyConst::NAMESPACE] = "test";             // 空间, 同时也是权限校验: 环境（必填）
    ...
    /*
    // 注册HelloServer
    if(!naming_service->registerInstance(service_name, "group1", instance)){
        std::cout << "平台注册失败, 服务:" << service_name << std::endl;
        exit(0);
    }
     */
    ...
    // 注册HelloServer
    if(!naming_service->registerInstance(service_name, ip, port)){
        std::cout << "平台注册失败, 服务:" << service_name << std::endl;
        exit(0);
    }
