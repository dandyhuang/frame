...
#include "thirdparty/monitor_sdk_common/storage/file/mock_file.h"
...
TEST(MockFile, Open) {
    ...
    COMMON_FILE_EXPECT_CALL(*f1, Tell())
...
TEST(MockFile, Unregister) {
    ...
    // destructed.
    EXPECT_EQ(0, f2->Tell());
    COMMON_FILE_EXPECT_CALL(*f2, Tell()).WillRepeatedly(Return(1));
    EXPECT_EQ(1, f2->Tell());
}
...
TEST(MockFile, Read)
{
    ...
    COMMON_FILE_EXPECT_CALL(*file, Read(_, _))
...
TEST(MockFile, Write)
{
    ...
    COMMON_FILE_EXPECT_CALL(*file, Write(_, _))
        .WillOnce(Return(5))
        .WillRepeatedly(Return(-1));
...
TEST(MockFile, ReadLine)
{
    ...
    COMMON_FILE_EXPECT_CALL(*file, ReadLine(_, _))
...
TEST(MockFile, Flush)
{
    ...
    COMMON_FILE_EXPECT_CALL(*file, Flush())
...
TEST(MockFile, Close)
{
    ...
    COMMON_FILE_EXPECT_CALL(*file, Close())
        .WillOnce(Return(true))
        .WillRepeatedly(Return(false));
...
TEST(MockFile, Seek)
{
    ...
    COMMON_FILE_EXPECT_CALL(*file, Seek(_, _))
        .WillOnce(Return(true))
        .WillRepeatedly(Return(false));
...
TEST(MockFile, Tell)
{
    ...
    COMMON_FILE_EXPECT_CALL(*file, Tell())
        .WillOnce(Return(1000))
        .WillRepeatedly(Return(-1));
