...
#ifndef COMMON_STORAGE_FILE_FILE_H
#define COMMON_STORAGE_FILE_FILE_H
#pragma once
...
class File {
    COMMON_DECLARE_UNCOPYABLE(File);
...
public:
    ...
    virtual bool ReadLine(std::string* line, size_t max_size = -1) = 0;
...
class FileSystem {
    COMMON_DECLARE_UNCOPYABLE(FileSystem);
protected:
    FileSystem();
    virtual ~FileSystem();
public:
...
COMMON_CLASS_REGISTRY_DEFINE_SINGLETON(file_system, FileSystem);
...
#define COMMON_REGISTER_FILE_SYSTEM(prefix, class_name) \
...
#define COMMON_GET_FILE_SYSTEM(prefix) \
    COMMON_CLASS_REGISTRY_GET_SINGLETON(file_system, prefix)

// Count of registed file systems.
#define COMMON_FILE_SYSTEM_COUNT() COMMON_CLASS_REGISTRY_CLASS_COUNT(file_system)

// Get file systems name by index
#define COMMON_FILE_SYSTEM_NAME(i) COMMON_CLASS_REGISTRY_CLASS_NAME(file_system, i)

#endif // COMMON_STORAGE_FILE_FILE_H
