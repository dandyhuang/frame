...
COMMON_REGISTER_FILE_SYSTEM("local", LocalFileSystem);
...
{
    const size_t cache_size = 4095;
    line->clear();
    while (1) {
        size_t expect_size = max_size > cache_size ? (cache_size + 1) : (max_size + 1);
        std::string tmp(expect_size, '\0');
        char *p = fgets(&tmp[0], expect_size, m_fp);
        if (!p) {
          if (line->empty()) return false;
          break;
        }
        size_t read_len = strnlen(p, expect_size);
        tmp.resize(read_len);
        if (line->empty()) line->swap(tmp);
        else line->append(tmp);
        max_size -= read_len;
        if (max_size == 0 || p[read_len - 1] == '\n') break;
    }
