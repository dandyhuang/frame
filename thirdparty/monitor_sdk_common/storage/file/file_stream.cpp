...
FileStream::FileStream() : m_fp(NULL) {}
...
bool FileStream::IsExist(const ::std::string& file_path, FileErrorCode* error_code) {
    SetErrorCode(error_code, kFileSuccess);
    return common::IsExist(file_path);
}

bool FileStream::IsDir(const ::std::string& file_path, FileErrorCode* error_code) {
    SetErrorCode(error_code, kFileSuccess);
    return common::IsDir(file_path);
}

int32_t FileStream::CreateDir(const ::std::string& file_path, FileErrorCode* error_code) {
    SetErrorCode(error_code, kFileSuccess);
    return CreateDirWithRetry(file_path) ? 0 : -1;
}

int32_t FileStream::Remove(const ::std::string& file_path, FileErrorCode* error_code) {
    SetErrorCode(error_code, kFileSuccess);
    return RemoveLocalFile(file_path) ? 0 : -1;
}

int32_t FileStream::Rename(const ::std::string& src_file_path,
                           const ::std::string& dest_file_path,
                           FileErrorCode* error_code) {
    if (MoveLocalFile(src_file_path, dest_file_path)) {
        SetErrorCode(error_code, kFileSuccess);
        return 0;
    } else {
