...
#ifndef COMMON_STORAGE_FILE_MOCK_FILE_H
#define COMMON_STORAGE_FILE_MOCK_FILE_H
#pragma once
...
class FileSystemMock : public FileSystem {
    COMMON_DECLARE_UNCOPYABLE(FileSystemMock);
public:
    FileSystemMock();
    ~FileSystemMock();
...
#define COMMON_FILE_EXPECT_CALL(file, call) \
    EXPECT_CALL(*::common::FileMock::FromFile(&(file), __FILE__, __LINE__), \
                call)

#define COMMON_FILE_ON_CALL(file, call) \
    ON_CALL(*::common::FileMock::FromFile(&(file), __FILE__, __LINE__), \
            call)

#endif // COMMON_STORAGE_FILE_MOCK_FILE_H
