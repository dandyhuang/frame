...
// Author: pittzhao413@163.com(ZhaoPu/11091180)
// date: 2019-11-12
...
namespace Serio {
#if SERIO_SIZE == 8
/// Data type of the size of containers and such that is serialized and
/// deserialized.
using Size = uint64_t;
#elif SERIO_SIZE == 4
/// Data type of the size of containers and such that is serialized and
...
void serialize(const T& data, char* buffer) {
  if (sizeof(T) == 1)
    ...
    throw std::runtime_error(
        "Basic data type is not supported for serialization");
}
...
void deserialize(T& data, const char* buffer) {
  if (sizeof(T) == 1)
    ...
    throw std::runtime_error(
        "Basic data type is not supported for deserialization");
}
}  // namespace Number
...
///
/// This class wraps the raw arrays so they can be serialized and deserialized
/// too.
///
/// Example:
/// @code
...
struct Array {
  size_t size;
  T* data;
  inline Array(size_t size, const T* data)
      : size(size), data(const_cast<T*>(data)) {}
};

/// @brief Processes higher structures for serialization and size calculation.
///
/// This class breaks higher structurs into simple elements and passes them on
/// to the class that handles them to calculate size of data or serialize it.
...
class SerializerOps {
 ...
 public:
  template <typename T>
  inline typename std::enable_if<std::is_class<T>::value, Derived&>::type
  operator<<(const T& C) {
    C._serialize(This());
    return This();
  }
  ...
  inline Derived& operator<<(const std::queue<T, Sequence>& C) {
    const auto* D =
        reinterpret_cast<const Impl::DerivedQueue<T, Sequence>*>(&C);
  ...
  inline Derived& operator<<(const std::priority_queue<T, Sequence>& C) {
    const auto* D =
        reinterpret_cast<const Impl::DerivedPQueue<T, Sequence>*>(&C);
    return iteratable(D->c);
  }
  ...
  inline Derived& operator<<(const std::stack<T, Sequence>& C) {
    const auto* D =
        reinterpret_cast<const Impl::DerivedStack<T, Sequence>*>(&C);
    return iteratable(D->c);
  }
...
///
/// This class breaks higher structurs into simple elements and passes them on
/// to the class that handles them to deserialize the data.
template <typename Derived>
...
class CalculatorBase {
 ...
 public:
  ...
  inline typename std::enable_if<
      std::is_arithmetic<T>::value || std::is_enum<T>::value, Derived&>::type
  operator<<(const T& C) {
    size += sizeof(C);
    return This();
  }
...
/// Calculates the size of unlimited number of input arguments (serializable
/// data types).
///
/// Example:
/// @code
...
/// Serializes and unlimited number of input arguments (serializable data
/// types).
///
/// Example:
/// @code
...
inline size_t fill(char* data, const Head& head, Tail&&... tail) {
  return size_t(
      Serializer(data).process(head, std::forward<Tail>(tail)...).buffer -
      data);
}

/// Deserializes and unlimited number of input arguments (deserializable data
/// types).
///
/// Example:
/// @code
...
inline size_t deserialize(const char* data, Head& head, Tail&&... tail) {
  return size_t(
      Deserializer(data).process(head, std::forward<Tail>(tail)...).buffer -
      data);
}
...
/// Reads data from a file and deserializes and unlimited number of input
/// arguments (deserializable data types).
///
/// Example:
/// @code
/// int A, B;
