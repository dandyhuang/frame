...
class SSTableReaderSet {
    COMMON_DECLARE_UNCOPYABLE(SSTableReaderSet);
...
public:
    SSTableReaderSet(const std::string &set_id, const std::string &sharding_policy, int num_shard)
        ...
        if (!sharding_policy_.empty()) {
            sharding_man_.reset(COMMON_CREATE_SHARDING_POLICY(sharding_policy));
...
private:
    ...
    common::scoped_ptr<ShardingPolicy> sharding_man_;
};
...
class MergedIterator : public SSTableReader::Iterator {
    COMMON_DECLARE_UNCOPYABLE(MergedIterator);
