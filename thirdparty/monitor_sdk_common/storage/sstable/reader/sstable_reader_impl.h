...
#ifndef COMMON_STORAGE_SSTABLE_READER_SSTABLE_READER_IMPL_H
#define COMMON_STORAGE_SSTABLE_READER_SSTABLE_READER_IMPL_H
...
class SSTableReader::Impl {
    COMMON_DECLARE_UNCOPYABLE(Impl);
...
public:
    ...
    void IterMetaData(common::Closure<bool(const std::string&, const std::string&)>* callback);

    int EntryCount() const {
        return file_trailer_->entry_count();
    }

    common::scoped_ptr<hfile::FileTrailer> file_trailer_;
    common::scoped_ptr<hfile::DataIndex> data_index_;
    std::string path_;

private:
    common::scoped_ptr<hfile::FileInfo> file_info_;
    uint32_t buffer_size_;

    common::Mutex mutex_;  // protects file_base_
    common::scoped_ptr<File> file_base_;
};

}  // namespace common

#endif  // COMMON_STORAGE_SSTABLE_READER_SSTABLE_READER_IMPL_H
