...
#include "thirdparty/monitor_sdk_common/base/stl_util.h"
...
#include "thirdparty/monitor_sdk_common/storage/sstable/test/test_util.h"
...
TEST(SingleSSTableWriter, BuildSingleBlockFile) {
    ...
    common::scoped_ptr<SSTableReader::Iterator> iter(sstable->NewIterator());
...
TEST(SingleSSTableWriter, BuildMultiBlock) {
    ...
    common::Closure<bool(const std::string &, const std::string &)> *callback =  // NOLINT
                    common::NewPermanentClosure(TestSmallMetaData);
...
void TestSSTableWriter(SSTableWriter *builder,
    ...
    common::scoped_ptr<SSTableReader> sstable(SSTableReader::Open(sstable_path, type));
    EXPECT_TRUE(sstable.get());
    common::scoped_ptr<SSTableReader::Iterator> iter(sstable->NewIterator());
    ...
    common::Closure<bool (const std::string &, const std::string &)> *callback =
    common::NewPermanentClosure(TestMetaData);
    sstable->IterateMetaData(callback);
    delete callback;
}
...
void TestSSTableWriterSeek(SSTableWriter *builder, const std::string &sstable_path,
    ...
    for (int i = 0; i < test_number; ++i) {
        std::string key = GenKey(i, key_mod);
        std::string value = GenValue(i, key_mod);
        common::scoped_ptr<SSTableReader::Iterator> iter(sstable->Seek(key));
...
TEST(ShardingSSTableWriterTest, Normal) {
    ...
    for (int i = 0; i < kTestNum; ++i) {
        ...
        for (int j = 0; j < 3; ++j) {
            ...
            common::scoped_ptr<SSTableReader::Iterator> iter(sstables[j]->Seek(key));
            if (iter->key() == key) {
                ++count;
