...
bool LibcurlWrapper::set_timeout(int timeout_ms) {
...
bool LibcurlWrapper::send_request(
    ...
    if (_timeout_ms > 0) {
        curl_easy_setopt(_curl, CURLOPT_TIMEOUT, static_cast<long>(_timeout_ms));
    }
...
bool LibcurlWrapper::send_get_request(
    ...
    if (_timeout_ms > 0) {
        curl_easy_setopt(_curl, CURLOPT_TIMEOUT_MS, static_cast<long>(_timeout_ms));
    }
