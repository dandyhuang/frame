...
std::shared_ptr<VepThirdPartyRelatedClient> vep_related_client;   //定义客户端
...
int main(int argc, char** argv){
    ...
    //2.初始化
    vep_related_client = std::make_shared<VepThirdPartyRelatedClient>(position_type, layer_code);
    ...
    //ResultVo<VepLabHitInfo> result_vo = vep_related_client->abValuesWithSeg(layer_code, traffic_position_id, tag);
    //ResultVo<VepLabHitInfo> result_vo = vep_related_client->abValueWithSegAndDiscardTag(layer_code, traffic_position_id, tag);
    //ResultVo<VepLabHitInfo> result_vo = vep_related_client->abValues(layer_code, traffic_position_id, tag);           //【即将弃用，新接口为abValuesWithSeg】
    //ResultVo<VepLabHitInfo> result_vo = vep_related_client->abValues(layer_code, traffic_position_id, shunt, tag);    // 通用接口api
