...
#ifndef COMMON_BASE_CLASS_REGISTRY_CLASS_REGISTRY_H
#define COMMON_BASE_CLASS_REGISTRY_CLASS_REGISTRY_H
...
// COMMON_CLASS_REGISTRY_DEFINE Make a unique type for a given registry_name.
...
#define COMMON_CLASS_REGISTRY_DEFINE(registry_name, base_class_name) \
    struct registry_name##RegistryTag: \
        public ::common::ClassRegistryTagBase<base_class_name> {};

#define COMMON_CLASS_REGISTRY_DEFINE_SINGLETON(registry_name, base_class_name) \
    struct registry_name##RegistryTag: \
        public ::common::ClassRegistrySingletonTagBase<base_class_name> {};

// User could select one of following two versions of
// COMMON_CLASS_REGISTRY_OBJECT_CREATOR, with or without singleton, but couldn't use
...
//
#define COMMON_CLASS_REGISTRY_REGISTER_CLASS(registry_name, \
                                           base_class_name, \
                                           entry_name_as_string, \
                                           class_name) \
    static ::common::ClassRegisterer<registry_name##RegistryTag> \
        COMMON_PP_JOIN(g_object_creator_registry_##class_name, __LINE__)( \
            entry_name_as_string, \
            &common::ClassRegistry_NewObject<base_class_name, class_name>)

#define COMMON_CLASS_REGISTRY_REGISTER_CLASS_SINGLETON(registry_name, \
                                                     base_class_name, \
                                                     entry_name_as_string, \
                                                     class_name) \
    static ::common::ClassRegisterer<registry_name##RegistryTag> \
        COMMON_PP_JOIN(g_object_creator_registry_##class_name, __LINE__)( \
            entry_name_as_string, \
            &::common::ClassRegistry_GetSingleton<class_name, registry_name##RegistryTag>)

// Create object from registry by name.
// Namespace prefix is required for registry_name if it is defined in different
// namespace
#define COMMON_CLASS_REGISTRY_CREATE_OBJECT(registry_name, entry_name_as_string) \
    ::common::ClassRegistryInstance<registry_name##RegistryTag>().CreateObject(entry_name_as_string)

// Get object singleton from registry by name.
#define COMMON_CLASS_REGISTRY_GET_SINGLETON(registry_name, entry_name_as_string) \
    ::common::ClassRegistryInstance<registry_name##RegistryTag>().GetSingleton(entry_name_as_string)

// Obtain the number of classes registered to the registry.
#define COMMON_CLASS_REGISTRY_CLASS_COUNT(registry_name) \
    ::common::ClassRegistryInstance<registry_name##RegistryTag>().ClassCount()

// Obtain class name by index for the registry.
#define COMMON_CLASS_REGISTRY_CLASS_NAME(registry_name, i) \
    ::common::ClassRegistryInstance<registry_name##RegistryTag>().ClassName(i)

#endif // COMMON_BASE_CLASS_REGISTRY_CLASS_REGISTRY_H
