...
#ifndef COMMON_ENCODING_HEX_H
#define COMMON_ENCODING_HEX_H
...
public:
    ...
    inline static std::string EncodeBuffer(
        const void* data, size_t size,
        bool uppercase = false
    )
    {
    ...
    DEPRECATED_BY(EncodeBuffer)
    inline static std::string EncodeString(
        const void* data, size_t size,
        bool uppercase = false)
    {
        return EncodeBuffer(data, size, uppercase);
    }
