...
#include "thirdparty/monitor_sdk_common/net/http/message.h"
#include "thirdparty/monitor_sdk_common/net/mime/mime.h"
                         ...
                         ErrorCode *error)
{
    ...
    try {
        ...
        ret = task.ProcessRequest(url, options, &request, response);
       *error = task.GetLastError();
    } catch (...) {
        VLOG(4) << "Construct DownloadTask failed, stream socket error.";
       *error = ERROR_FAIL_TO_SEND_REQUEST;
    }
