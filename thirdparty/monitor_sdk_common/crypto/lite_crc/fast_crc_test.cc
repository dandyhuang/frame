...
#else
    tv->tv_sec = static_cast<int64_t>(time(NULL));
    return 0;
#endif
}
...
void test_crc8_speed() {
    ...
    float f_cost = static_cast<float>(cost) / static_cast<float>(1000000);
    float speed = static_cast<float>(buff_len) / static_cast<float>(1024*1024) / f_cost;
...
TEST(TestFastCRC, FastCRC) {
    ...
    float cost = static_cast<float>(cost_usecs) / static_cast<float>(1000000);

    float speed = (static_cast<float>(max_buff) / static_cast<float>(1024*1024)) / cost;
    printf("cost time:%5.2f seconds, for FastCRC data length:%5.2f M Bytes\r\n", cost,
                                static_cast<float>(max_buff) / static_cast<float>(1024*1024));
