...
// Comment out the following functions for initializing the CRC32 table, as it
    ...
    for (i = 0; i <= 0xFF; i++)
    {
        ...
        for (j = 0; j < 8; j++)
        {
            g_crc32_table[i] = (g_crc32_table[i] << 1) ^
                (g_crc32_table[i] & (1 << 31) ? polynomial : 0);
        }
...
uint32_t UpdateCRC32(const void* data, size_t size, const uint32_t old_crc)
{
...
// Comment this function out until we unify naming of all hash functions.
/*
uint16_t Hash16(const char * data, size_t size)
{
    uint32_t hash = 0;
    const uint8_t* ptr = reinterpret_cast<const uint8_t*>(data);
    for (size_t i = 1; i <= size; i++, ptr++) {
        hash += (*ptr) * 3 * i;
    }
    uint16_t high=hash>>16;
    uint16_t low=hash&0xFFFF;
    uint16_t hash16=high^low;
    if (hash16 == 0) {
        hash16 = 1;
    }
    return hash16;
