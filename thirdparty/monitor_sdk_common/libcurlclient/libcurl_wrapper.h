...
namespace common
{
    ...
    enum CURL_TIMEOUT_TYPE
    {
        CURL_TIMEOUT_SECOND_TYPE = 1,          // 秒
        CURL_TIMEOUT_MILLISECOND_TYPE = 2,     // 毫秒
    };

    static const int DEFAULT_CURL_TIMEOUT_MS{3000};
    ...
    public:
        ...
        virtual bool set_timeout(int timeout, CURL_TIMEOUT_TYPE type);
		
        virtual bool set_connect_timeout(int timeout, CURL_TIMEOUT_TYPE type);
        ...
        virtual bool send_request(const std::string& url, const std::map<std::string, std::string>& parameters, int32_t* http_status_code
                                  , std::string* http_header_data, std::string* http_response_data);

        virtual bool send_post_request(
                const std::string &url, const std::string &body, int32_t *http_status_code
                , std::string *http_header_data, std::string *http_response_data);
    ...
    private:
        ...
        bool send_request(const std::string &url, const std::string &body, const std::string &method, int32_t *http_status_code, std::string *http_header_data
                          , std::string *http_response_data);
    ...
    private:
        ...
        int32_t _timeout_type;
		int32_t _connect_timeout_type;
